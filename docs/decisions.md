...
## Track A — 기능 추출 (① 테스트 대상 기능 추출, A·B 담당)

> Track B(제약 파서)는 별도 트랙이 아니라 이 트랙(기능 추출)의 일부라 여기로 합쳤습니다. Analyzer가 라우터·의존성·예외뿐 아니라 요청 제약조건(`Constraint`)까지 한 파이프라인에서 함께 추출하기 때문입니다.
...
- **트레이드오프**: 없음 — 이미 검증된 헬퍼를 재사용하는 것이라 새 리스크 없이 버그만 닫힘. 겸사겸사 `router_graph.py`의 `_mount_targets` 중복 호출, `extractors/*.py`의 함수-로컬 import 반복 등 PR #27 리뷰에서 지적됐지만 재적용 과정에서 놓쳤던 nit들도 같이 정리.
- **관련 이슈/PR**: #41, #42

### Analyzer 재적용 시 회귀 테스트로 재검증하는 절차 확립
- **무엇을 결정했나**: PR #27(최초 구현) → PR #30(리뷰 대응 지연으로 임시 revert) → PR #35(develop 최신 기준 재적용 + 리뷰 피드백 반영)로 이어지는 과정에서, 재적용 시마다 "리뷰에서 지적됐던 항목이 실제로 해결됐는지"를 코드만 보고 판단하지 않고 재현 코드를 직접 실행해서 확인하는 절차를 거침.
- **왜** (고려했던 대안 포함): 재적용 PR의 설명만 보면 리뷰 지적사항이 해결된 것처럼 보였지만("return 문으로 범위 제한"), 실제로는 부분적으로만 해결돼 있었음(위 항목). 코드 리뷰만으로는 "부분적으로 고쳐진 버그"를 놓치기 쉬워서, 의심되는 지점은 실제 실행 결과(수정 전/후 비교)로 재검증하는 걸 원칙으로 삼음.
- **트레이드오프**: 리뷰에 걸리는 시간이 늘어나지만, 이미 한 번 리뷰했던 코드라고 안심하고 넘어가는 것보다 안전하다고 판단.
- **관련 이슈/PR**: #27, #30, #35, #41, #42
